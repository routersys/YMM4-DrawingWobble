...
public sealed class TextsTests
{
    ...
    {
        var neutral = Texts.ResourceManager.GetString(nameof(Texts.DrawingWobble), CultureInfo.InvariantCulture);

        Assert.Equal("作画ゆれ", neutral);
        Assert.Equal(neutral, Texts.ResourceManager.GetString(nameof(Texts.DrawingWobble), CultureInfo.GetCultureInfo("ja-JP")));
    }
